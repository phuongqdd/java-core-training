    ...
    public UserCourse save(UserCourse userCourse) {
        if(userCourse.getId() == null){
            ...
            jdbcTemplate.update(con -> {
                ...
                ps.setLong(3, userCourse.getCourseId());
                ps.setLong(4, userCourse.getUserId());
                ps.setInt(5, userCourse.getRoleId());
                return ps;
            }, keyHolder);
    ...
    public List<UserCourseResponse> findAllByCourseForUser(Long userId) {
        String sql = """
                SELECT c.course_id, c.name as courseName, c.description, c.thumbnail_url,
                        uc.enrolled_at, uc.is_owner
                FROM user_course uc JOIN course c
                    ON uc.course_id = c.course_id
                WHERE uc.user_id = ?
                """;

        List<UserCourseResponse> responses = jdbcTemplate.query(sql,
                new BeanPropertyRowMapper<>(UserCourseResponse.class),
                userId);

        for(UserCourseResponse tmp : responses){
            String sql1 = """
                    SELECT role_id
                    FROM user_course
                    WHERE user_id = ? AND course_id = ?
                    """;
            Integer roleId = jdbcTemplate.queryForObject(sql1, Integer.class, userId, tmp.getCourseId());
